<template>
  <div class="csv-reader">
    <div class="header-actions">
      <button class="back-btn" @click="closeModal">🏠 กลับหน้าหลัก</button>
    </div>
    ...
    <div class="upload-section">
      ...
      <button class="save-btn" @click="saveToDatabase" :disabled="!jsonData">
        💾 บันทึกลงฐานข้อมูล
      </button>
...
const { updateMeetingInfo, updateProjects, saveData: saveStoreData } = store;
...
const convertCSVToJSON = (csvData: string[][]): any => {
  ...
  let projectSequence = 1; // ตัวแปรสำหรับนับลำดับโครงการ
  ...
  for (let i = 0; i < csvData.length; i++) {
    ...
    if (col1 === "ลำดับ" && col2 === "ระบบงาน" && col3.trim() !== "") {
      ...
      currentProject = {
        ลำดับ: projectSequence, // ใช้ลำดับที่เพิ่มขึ้นเรื่อยๆ
        ระบบงาน: projectName,
        ผู้รับผิดชอบ: "",
        PM: "",
        ปัญหาที่พบ: [],
      ...
      projectSequence++; // เพิ่มลำดับสำหรับโครงการถัดไป
  ...
  const result: any = {
    meetingInfo: {
      ...
      date: "สัปดาห์ที่ 0 วันที่ 00/00/0000",
    },
    projectStats: {
...
const saveToDatabase = async () => {
  ...
  try {
    ...
    store.meetingData = jsonData.value;

    // บันทึกลงฐานข้อมูล
    await saveStoreData();

    showMessage("✅ เซฟทับลงฐานข้อมูลสำเร็จ!", "success");

    // ปิด modal
    closeModal();
  } catch (error) {
    showMessage(`❌ เกิดข้อผิดพลาดในการเซฟ: ${error}`, "error");
