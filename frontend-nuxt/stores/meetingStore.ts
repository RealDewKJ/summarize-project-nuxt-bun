...
export const useMeetingStore = defineStore("meeting", () => {
  ...
  const meetingData = ref<MeetingData>({
    meetingInfo: {
      title: "สรุปการประชุมประจำสัปดาห์ทีมพัฒนา",
      date: "สัปดาห์ที่ 00 วันที่ 00/00/0000",
    },
    projectStats: {
      total: 44,
      inProgress: 30,
    ...
    issuesDetails: [
      {
        ...
        description:
          "งานชุมชน เมนูคัดกรองกลุ่มเสี่ยง วาง UI ใหม่เรียบร้อยแล้ว กำลังทำส่วนบันทึกข้อมูล",
        priority: "high",
      },
      {
      ...
      },
      {
        project: "MyAtlas",
        description:
          "เพิ่มแบบประเมินหอบหืด และไฟล์เสียงอ่านคำถามแล้ว รอปรับแก้ไขตาม requirement ใหม่",
        priority: "medium",
      },
      {
      ...
      },
      {
        ...
        description:
          "เมื่อกดเลือก lab แล้วกดเพิ่มในกราฟ เส้นกราฟแสดงแต่ชือ series ไม่เปลี่ยนตามชื่อ lab ให้",
        priority: "high",
      },
      {
        project: "HOSxP ระบบเยี่ยมบ้าน",
        description:
          "ส่งเยี่ยมบ้าน จากหน้าจอ IPD, ลด ToKen AI ลงมาอีก โดยการใช้ภาษา English แทน ภาษาไทย",
        priority: "high",
      },
      {
        project: "Tools รพ.กระทุ่มแบน",
        description:
          "แปลงรูป tiff เป็น jpg - ลองทำฟังก์ชันพื้นฐานขึ้นมาดึงและแปลงบน code dephi ไม่สำเร็จ",
        priority: "medium",
      },
      {
        project: "AtlasHomeCare (เมนูงานชุมชน → สั่ง LAB)",
        description:
          "ได้รับ requirement ให้แก้ไขเพิ่มเติม 1. เมนูตั้งค่า AuthenCode 2. ช่องค้นหา CID, HN, ชื่อ 3. LAB ยกเลิกใบสั่ง 4. ตรวจสอบข้อมูลฟิลด์บันทึกเปิด visit",
        priority: "high",
      },
      {
        project:
          "Dashboard กลางสำหรับโรงพยาบาล (Monitor ข้อมูลตามประกาศของ สปสช.)",
        description:
          "flutter ไม่สามารถนำเข้าไฟล์ Excel .xls ไม่ได้เนื่องจากเป็นเวอร์ชันเก่า รองรับแค่ .xlsx",
        priority: "medium",
      },
      {
        project: "ทดลองงาน",
      ...
      },
      {
        ...
        description:
          "เพิ่มหน้าจอ Refer In แล้วกำลังทดสอบว่าสามารถเพิ่มข้อมูลได้ถูกต้องหรือไม่",
        priority: "medium",
      },
    ],
    codeReview: [
      ...
      {
        project:
          "NHIP - ขอเพิ่มฟังก์ชันตรวจสอบการลงค่า Vital Signs (One Stop Service)",
  ...
  const inProgressCount = computed(
    () => meetingData.value.projectStats.inProgress
  );
  const completedCount = computed(
    () => meetingData.value.projectStats.completed
  );
  ...
  const loadData = async () => {
    ...
    loadingMessage.value = "กำลังเชื่อมต่อกับเซิร์ฟเวอร์...";
    ...
    try {
      ...
      loadingMessage.value = "กำลังโหลดข้อมูลจากฐานข้อมูล...";
      loadingProgress.value = 30;

      const response = await $fetch<MeetingData>(`${apiUrl}/api/meeting`);

      loadingMessage.value = "กำลังประมวลผลข้อมูล...";
      loadingProgress.value = 70;

      meetingData.value = response;

      loadingMessage.value = "เสร็จสิ้น!";
      loadingProgress.value = 100;

      console.log("✅ โหลดข้อมูลจาก API สำเร็จ:", response);

      // หน่วงเวลาเล็กน้อยเพื่อให้เห็น loading เสร็จ
      await new Promise((resolve) => setTimeout(resolve, 500));
    } catch (error) {
      console.error("❌ Error loading data from API:", error);
      console.log("⚠️ พยายามโหลดจาก data.json แทน...");

      loadingMessage.value = "กำลังโหลดจากไฟล์สำรอง...";
      loadingProgress.value = 50;

      // Fallback: ลองโหลดจาก data.json
      try {
        const response = await $fetch<MeetingData>("/data.json");
        meetingData.value = response;

        loadingMessage.value = "เสร็จสิ้น!";
        loadingProgress.value = 100;

        console.log("✅ โหลดข้อมูลจาก data.json สำเร็จ");

        // หน่วงเวลาเล็กน้อยเพื่อให้เห็น loading เสร็จ
        await new Promise((resolve) => setTimeout(resolve, 500));
      } catch (fallbackError) {
        console.error("❌ Error loading data.json:", fallbackError);
        // ใช้ข้อมูลเริ่มต้นถ้าโหลดไม่ได้ทั้ง API และ JSON
        meetingData.value = getDefaultData();

        loadingMessage.value = "ใช้ข้อมูลเริ่มต้น";
        loadingProgress.value = 100;

        console.log("📝 ใช้ข้อมูลเริ่มต้น");
  ...
  const updateProjects = (
    category: keyof typeof meetingData.value.projects,
    projects: string[]
  ) => {
  ...
  const updateProjectStatus = (
    projectName: string,
    newStatus: keyof typeof meetingData.value.projects
  ) => {
    // ลบจากสถานะเดิม
    Object.keys(meetingData.value.projects).forEach((status) => {
      const index =
        meetingData.value.projects[
          status as keyof typeof meetingData.value.projects
        ].indexOf(projectName);
      if (index > -1) {
        meetingData.value.projects[
          status as keyof typeof meetingData.value.projects
  ...
  const saveData = async () => {
    ...
    try {
      ...
      loadingMessage.value = "กำลังส่งข้อมูลไปยังเซิร์ฟเวอร์...";
      loadingProgress.value = 50;

      const result = await $fetch(`${apiUrl}/api/meeting`, {
        method: "POST",
        body: meetingData.value,
      });

      loadingMessage.value = "กำลังบันทึกลงฐานข้อมูล...";
      loadingProgress.value = 80;

      console.log("✅ บันทึกข้อมูลสำเร็จ:", result);

      loadingMessage.value = "บันทึกสำเร็จ!";
      loadingProgress.value = 100;
      ...
      // แสดง success modal
      successTitle.value = "บันทึกสำเร็จ!";
      successMessage.value = "ข้อมูลถูกบันทึกลงฐานข้อมูลเรียบร้อยแล้ว";
      ...
      loadingMessage.value = "กำลังเตรียมไฟล์สำรอง...";
      loadingProgress.value = 60;

      // Fallback: ดาวน์โหลดเป็นไฟล์ JSON
      console.log("⚠️ ไม่สามารถบันทึกผ่าน API ได้ ดาวน์โหลดเป็นไฟล์แทน");
      ...
      loadingMessage.value = "ดาวน์โหลดไฟล์สำรองแล้ว";
      loadingProgress.value = 100;
